---
title: "Offense Involvement Charts for the 2025 NFL Season (WRs, RBs, TEs)"
...
```{r}
all_players_2025 <- bind_rows(
  wr_clean_2025 %>% select(player_id, player_display_name, team, week, targets, target_share),
  rb_clean_2025 %>% select(player_id, player_display_name, team, week, targets, target_share),
  te_clean_2025 %>% select(player_id, player_display_name, team, week, targets, target_share)
)

season_target_share <- all_players_2025 %>%
...
#ggplot(season_target_share, aes(x = "", y = target_pct, fill = player_display_name)) +
#  geom_col(width = 1, color = "white") +
#  coord_polar(theta = "y") +
#  labs(title = "Season-Long Target Share Distribution",
#       fill = "Player") +
#  theme_void()
```
...
```{r}
all_ui <- fluidPage(
  titlePanel("Team Target Share Distribution – 2025 Season"),
  sidebarLayout(
    sidebarPanel(
      selectInput("pie_team_choice", "Select Team:",
                  choices = sort(unique(all_players_2025$team)))
    ),
    mainPanel(
...
ggplot(target_share, aes(x = reorder(player_label, target_share), y = target_share, fill = team)) +
  ...
  coord_flip() +
  labs(
    title = "Top 10 Target Share Leaders (2025)",
    x = "",
    y = "Target Share"
  ) +
