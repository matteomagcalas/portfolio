---
title: "Offense Involvement Charts for the 2024 NFL Season (WRs, RBs, TEs)"
...
```{r}
wr_weekly_2024 <- load_player_stats(seasons = 2024, stat_type = "weekly") %>%
  filter(position == "WR", season_type == "REG")


wr_clean_2024 <- wr_weekly_2024 %>%
  select(
...
```{r}
all_weeks_wr <- 1:max(wr_clean_2024$week, na.rm = TRUE)

wr_skeleton <- expand_grid(
  player_id = unique(wr_clean_2024$player_id),
  week = all_weeks_wr
)

wr_expanded <- wr_skeleton %>%
  left_join(wr_clean_2024, by = c("player_id", "week")) %>%
  group_by(player_id) %>%
...
```{r}
wr_ui <- fluidPage(
  titlePanel("WR Trends - 2024 Season"),
...
```{r}
rb_weekly_2024 <- load_player_stats(seasons = 2024, stat_type = "weekly") %>%
  filter(position == "RB", season_type == "REG")

rb_clean_2024 <- rb_weekly_2024 %>%
  select(
    player_id, player_display_name, week, team, opponent_team,
...
```{r}
all_weeks_rb <- 1:max(rb_clean_2024$week, na.rm = TRUE)

rb_skeleton <- expand_grid(
  player_id = unique(rb_clean_2024$player_id),
  week = all_weeks_rb
)

rb_expanded <- rb_skeleton %>%
  left_join(rb_clean_2024, by = c("player_id", "week")) %>%
  group_by(player_id) %>%
  fill(player_display_name, team, opponent_team, .direction = "downup") %>%
  ungroup()
...
```{r}
rb_ui <- fluidPage(
  titlePanel("RB Trends - 2024 Season"),
...
```{r}
te_weekly_2024 <- load_player_stats(seasons = 2024, stat_type = "weekly") %>%
  filter(position == "TE", season_type == "REG")

te_clean_2024 <- te_weekly_2024 %>%
  select(
    player_id, player_display_name, week, team, opponent_team,
...
```{r}
all_weeks_te <- 1:max(te_clean_2024$week, na.rm = TRUE)

te_skeleton <- expand_grid(
  player_id = unique(te_clean_2024$player_id),
  week = all_weeks_te
)

te_expanded <- te_skeleton %>%
  left_join(te_clean_2024, by = c("player_id", "week")) %>%
  group_by(player_id) %>%
  fill(player_display_name, team, opponent_team, .direction = "downup") %>%
  ungroup()
...
```{r}
te_ui <- fluidPage(
  titlePanel("TE Trends - 2024 Season"),
...
```{r}
all_players_2024 <- bind_rows(
  wr_clean_2024 %>% select(player_id, player_display_name, team, week, targets, target_share),
  rb_clean_2024 %>% select(player_id, player_display_name, team, week, targets, target_share),
  te_clean_2024 %>% select(player_id, player_display_name, team, week, targets, target_share)
)

season_target_share <- all_players_2024 %>%
  group_by(player_id, player_display_name, team) %>%
  summarise(total_targets = sum(targets, na.rm = TRUE), .groups = "drop") %>%
  mutate(target_pct = total_targets / sum(total_targets))
```

```{r}
all_ui <- fluidPage(
  titlePanel("Team Target Share Distribution – 2024 Season"),
  sidebarLayout(
    sidebarPanel(
      selectInput("pie_team_choice", "Select Team:",
                  choices = sort(unique(all_players_2024$team)))
    ),
    mainPanel(
      plotOutput("target_pie", height = "600px")
    )
  )
)

all_server <- function(input, output, session) {
  
  output$target_pie <- renderPlot({
    req(input$pie_team_choice)
    
    season_target_share <- all_players_2024 %>%
      filter(team == input$pie_team_choice) %>%
      group_by(player_id, player_display_name) %>%
      summarise(total_targets = sum(as.numeric(targets), na.rm = TRUE), .groups = "drop") %>%
      filter(total_targets > 0) %>%
      arrange(desc(total_targets)) %>%
      mutate(target_pct = total_targets / sum(total_targets),
             label = paste0(player_display_name, " ", percent(target_pct, accuracy = 0.1)))
    
    ggplot(season_target_share, aes(x = "", y = target_pct, fill = reorder(player_display_name, -total_targets))) +
      geom_col(width = 1, color = "white") +
      coord_polar(theta = "y") +
      labs(
        title = paste("Season-Long Target Share Distribution –", input$pie_team_choice),
        fill = "Player"
      ) +
      theme_void(base_size = 14) +
      geom_text(aes(label = percent(target_pct, accuracy = 0.1)),
                position = position_stack(vjust = 0.5), size = 4, color = "black", check_overlap = TRUE) +
      guides(fill = guide_legend(reverse = FALSE))
  })
}

shinyApp(ui = all_ui, server = all_server)
```

```{r}
team_totals <- all_players_2024 %>%
  group_by(team) %>%
  summarise(team_targets = sum(as.numeric(targets), na.rm = TRUE), .groups = "drop")

target_share <- all_players_2024 %>%
  group_by(player_id, player_display_name, team) %>%
  summarise(total_targets = sum(as.numeric(targets), na.rm = TRUE), .groups = "drop") %>%
  left_join(team_totals, by = "team") %>%
  mutate(target_share = total_targets / team_targets) %>%
  arrange(desc(target_share)) %>%
  slice_head(n = 10) %>%
  mutate(player_label = paste0(player_display_name, " (", team, ")"))

ggplot(target_share, aes(x = reorder(player_label, target_share), y = target_share, fill = team)) +
  geom_col() +
  coord_flip() +
  labs(
    title = "Top 10 Target Share Leaders (2024)",
    x = "",
    y = "Target Share"
  ) +
